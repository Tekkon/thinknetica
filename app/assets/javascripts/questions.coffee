...
$ ->
  ...
  $(document).on('ajax:success', '.rb-vote', (e, data, status, xhr) ->
    ...
    $('#question-' + result.vote.votable_id + '-vote-result').html(JST['vote_result']({ vote: result.vote }))
  ...
  $(document).on('ajax:success', '.revote-link', (e, data, status, xhr) ->
    result = $.parseJSON(xhr.responseText)
    $('#question-' + result.vote.votable_id + '-vote').html(JST['vote_buttons']({ vote: result.vote }))
